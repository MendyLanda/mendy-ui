...
Merging creates a GitHub release and `vX.Y.Z` tag. Once the merged commit passes main CI, the publisher releases that exact commit to `latest`, followed by its development snapshot. A GitHub release alone is not proof that npm publishing succeeded; check **Publish npm package**.
