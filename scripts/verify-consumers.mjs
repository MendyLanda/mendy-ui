...
const registry = process.argv[2] ?? "https://ui.mendylanda.com";
...
function run(dir, args) {
  ...
  const result = spawnSync(process.execPath, [pnpm, ...args], { cwd: dir, env, stdio: "inherit" });
  if (result.status !== 0)
    throw new Error(`Consumer command failed: pnpm ${args.join(" ")} in ${dir}`);
}
