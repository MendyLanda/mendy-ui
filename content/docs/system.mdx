...
`FilterBar` shows suggestions when no visible applied filters remain. Set `suggestions="always"` to keep unapplied suggestions visible alongside applied chips, or `suggestions="never"` to hide them.
...
`renderEditor` receives typed `value`, `setValue`, `draft`, `setDraft`, `apply`, and `close`. `setValue` applies immediately and updates the draft; clearing the value closes its editor. Otherwise it stays open. `setDraft` changes only the local draft; `apply()` commits the draft and closes. Closing an editor discards an unfinished draft.
