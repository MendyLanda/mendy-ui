...
`filter.dateRange` uses the shadcn Calendar. Choose one day, or a start and end date, then Apply. A single day sets both endpoints to that date. Values are local calendar dates in `YYYY-MM-DD` format; your API decides how to interpret inclusive endpoints and time zones. Existing open-ended ranges keep their null endpoint until edited. Clear date uses the field's `clearValue`.
