...
Shared settings include `label`, `defaultValue`, `clearValue`, `isActive`, `normalize`, `validate`, `codec`, `urlKey`, `suggestion`, `summary`, `renderSummary`, `renderEditor`, `menu`, `hidden`, `disabled`, and `removable`. `searchLabel`, `editorLabel`, and `placeholder` customize editor labels. Application validation remains necessary at the API boundary.
